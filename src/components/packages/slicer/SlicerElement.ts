...
export class SlicerElement extends HTMLElement {
  ...
  constructor(parameters?: SlicerParameters) {
    ...
    this.#wordConstructor = parameters?.wordConstructor || Word
    this.#letterConstructor = parameters?.letterConstructor || Letter
  }
  ...
  protected connectedCallback() {
    ...
    this.#words = this.#originalText
      ...
      .split(' ')
      .map((text, index) => {
        const word = new this.#wordConstructor({
          index,
          text,
          letters: this.hasAttribute('letters'),
          lettersAcc,
          letterConstructor: this.#letterConstructor,
