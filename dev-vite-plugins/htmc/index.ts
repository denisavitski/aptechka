...
class HTMC {
  ...
  constructor(options?: HTMCOptions) {
    this.#srcPath = normalizePath(
      resolve(process.cwd(), options?.srcFolderName || 'src')
    )
    ...
    this.#plugin = {
      ...
      transformIndexHtml: {
        ...
        handler: async (html, ctx) => {
          ...
          if (isComponent) {
            let name = ctx
              .originalUrl!.split('/components/')[1]
              .split('?')[0]
              .split('#')[0]
  ...
  #transform() {
    ...
    do {
      ...
      components.forEach((component) => {
        ...
        const folderPath = this.#joinPaths(
          this.#srcPath,
          'components',
          name,
          '/'
        ...
        if (existsSync(folderPath)) {
          ...
          if (existsSync(indexHTMLPath)) {
            ...
            const componentEmptyElement = this.#dom.document.createElement(
              null!
            ) as Element
          ...
          const componentsNames = folderPath
            .split(this.#componentsPath)[1]
            .slice(1, -1)
            .split('/')
      ...
      components.length &&
      components.filter((c) => c.hasAttribute('not-found')).length !==
        components.length
    )
  }
      ...
      if (attr.name !== 'name' && attr.value) {
        elements.forEach((element) => {
          if (element.attributeName === attr.name) {
            const currentValue =
              element.element.getAttribute(element.attributeName) || ''
  ...
  #nest(component: Element, componentElement: Element) {
    let nestInserts = [...component.children].filter((element) =>
      element.hasAttribute('nest')
    )
    ...
    nestElements.forEach((nestElement) => {
      ...
      if (name && name !== 'default') {
        ...
        const inserts = nestInserts.filter(
          (insert) =>
            !insert.getAttribute('nest') ||
            insert.getAttribute('nest') === 'default'
        )
        inserts.forEach((insert) => insert.removeAttribute('nest'))
        nestElement.outerHTML =
          inserts.reduce((p, c) => p + c.outerHTML, '') + defaultInserts
      }
    })
  }
